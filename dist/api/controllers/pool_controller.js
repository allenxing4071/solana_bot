...
exports.getPoolStats = exports.getPoolsByToken = exports.getPoolsByDex = exports.getPoolDetails = exports.getAllPools = void 0;
...
const getAllPools = async (req, res) => {
    try {
        ...
        const dex = req.query.dex || 'all';
        // 获取所有池子
        let pools = pool_monitor_1.default.getKnownPools();
        // 应用搜索和过滤
        if (search) {
            pools = pools.filter(pool => (pool.tokenASymbol && pool.tokenASymbol.toLowerCase().includes(search.toLowerCase())) ||
 ...
 * @param res 响应对象
 */
const getPoolsByDex = async (req, res) => {
    try {
        ...
        const pools = pool_monitor_1.default.getKnownPools();
        // 过滤指定DEX的池子
        const filteredPools = pools.filter(p => p.dex.toLowerCase() === dexName.toLowerCase());
        res.status(200).json({
            success: true,
        ...
        logger_1.default.error('获取DEX流动性池失败', MODULE_NAME, {
 ...
 * @param res 响应对象
 */
const getPoolsByToken = async (req, res) => {
    try {
        ...
        const pools = pool_monitor_1.default.getKnownPools();
        // 过滤包含指定代币的池子
        const filteredPools = pools.filter(p => p.tokenAMint.toString() === mint ||
        ...
        logger_1.default.error('获取代币相关流动性池失败', MODULE_NAME, {
            error: error instanceof Error ? error.message : String(error)
        });
        res.status(500).json({
            success: false,
