...
exports.applySettings = exports.saveSettings = exports.getSettings = void 0;
...
const getSettings = async (_req, res) => {
    try {
        ...
        logger_1.default.error('获取系统设置失败', MODULE_NAME, {
            error: error instanceof Error ? error.message : String(error)
        });
        res.status(500).json({
            success: false,
...
const saveSettings = async (req, res) => {
    try {
        ...
        logger_1.default.error('保存系统设置失败', MODULE_NAME, {
            error: error instanceof Error ? error.message : String(error)
        });
        res.status(500).json({
            success: false,
...
const applySettings = async (req, res) => {
    ...
    try {
        ...
        logger_1.default.error('应用系统设置失败', MODULE_NAME, {
            error: error instanceof Error ? error.message : String(error)
        });
        res.status(500).json({
            success: false,
