...
const token_validator_1 = __importDefault(require("./token_validator"));
...
class OpportunityDetector {
    ...
    constructor() {
        ...
        try {
            // 从配置中加载设置
            // 就像读取船长的捕捞标准手册
            this.minLiquidityUsd = config_1.default.security.tokenValidation.minLiquidityUsd || 1000;
            this.maxInitialPriceUsd = config_1.default.security.tokenValidation.maxInitialPriceUsd || 0.01;
            // 记录配置加载信息
            // 就像在日志中记录使用的评估标准
            logger_1.default.debug('加载机会检测器配置', MODULE_NAME, {
            ...
            // 配置加载出错时使用默认值
            // 就像标准手册损坏时使用经验值
            logger_1.default.warn('加载配置时出错，使用默认值', MODULE_NAME, { error: err instanceof Error ? err.message : String(err) });
        ...
        logger_1.default.info('机会检测器初始化完成', MODULE_NAME);
    }
    /**
     * 初始化基础代币列表
     * 添加系统使用的主要交易货币
    ...
    initializeBaseTokens() {
        ...
        this.baseTokens.push({
            mint: new web3_js_1.PublicKey('So11111111111111111111111111111111111111112'), // 包装后的SOL地址
            symbol: 'SOL',
            name: 'Solana',
            decimals: 9,
            isVerified: true,
        ...
        // 就像记录银币的详细信息
        this.baseTokens.push({
            mint: new web3_js_1.PublicKey('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v'), // USDC地址
            symbol: 'USDC',
            name: 'USD Coin',
            decimals: 6,
            isVerified: true,
     ...
     */
    async detectOpportunity(poolInfo) {
        // 记录开始分析的日志
        // 就像在航海日志中记录发现新鱼群
        logger_1.default.debug('分析池子: ' + poolInfo.address.toBase58(), MODULE_NAME, {
            dex: poolInfo.dex,
        ...
        try {
            ...
            if (!targetToken || !baseToken) {
                logger_1.default.debug('跳过池子: 无法识别目标代币或基础代币', MODULE_NAME, {
            ...
            // 2. 验证目标代币
            // 就像检查这种鱼是否安全、合法可捕捞
            const validationResult = await token_validator_1.default.validateToken(targetToken);
            if (!validationResult.isValid) {
                logger_1.default.debug('跳过池子: 代币验证失败 - ' + (validationResult.reason || '未知原因'), MODULE_NAME, {
            ...
            if (!analysis.isValid) {
                logger_1.default.debug('跳过池子: 分析失败 - ' + (analysis.reason || '未知原因'), MODULE_NAME, {
                    token: targetToken.symbol || targetToken.mint.toBase58()
                });
                return null;
            }
            ...
            // 获取预估的输出数量
            // 就像估算能捕获的鱼量
            const estimatedOutAmount = this.estimateOutputAmount(poolInfo, baseToken, targetToken, config_1.default.trading.buyStrategy.maxAmountPerTrade);
            // 计算利润百分比
            ...
            // 记录发现交易机会的日志
            // 就像在航海日志中记录值得捕捞的鱼群
            logger_1.default.info('检测到交易机会: ' + (targetToken.symbol || targetToken.mint.toBase58()), MODULE_NAME, {
                dex: poolInfo.dex,
            ...
            // 处理错误
            // 就像处理评估过程中遇到的意外情况
            logger_1.default.error('分析池子时出错: ' + poolInfo.address.toBase58(), MODULE_NAME, {
    ...
    async identifyTokens(poolInfo) {
        ...
        // 丰富代币信息
        // 就像获取这种鱼的更多详细信息
        const enrichedTargetToken = await token_validator_1.default.enrichTokenInfo(targetToken);
     ...
     */
    async analyzePool(poolInfo, targetToken, baseToken) {
     ...
     * @returns 估算价格(美元)
     */
    async estimateTokenPrice(poolInfo, targetToken, baseToken) {
     ...
     * @returns 估算流动性(美元)
     */
    async estimateLiquidity(poolInfo, targetToken, baseToken) {
     ...
     * @returns 信心分数(0-1)
     */
    calculateConfidenceScore(poolInfo, targetToken, price, liquidity) {
        // 这里实际实现会更复杂，综合考虑多种因素
        // 以下为简化实现
        // 基础分数
        let score = 0.5;
     ...
     * @returns 优先级分数
     */
    calculatePriorityScore(analysis, poolInfo, targetToken) {
        // 这里实际实现会更复杂，综合考虑多种因素
        // 以下为简化实现
        // 基础分数 = 信心分数 * 100
     ...
     * @returns 估算滑点百分比
     */
    estimateSlippage(poolInfo, liquidityUsd) {
     ...
     * @returns 估算输出数量
     */
    estimateOutputAmount(poolInfo, inputToken, outputToken, amountIn) {
     ...
     * @returns 利润百分比
     */
    calculateProfitPercentage(estimatedPriceUsd, liquidityUsd) {
