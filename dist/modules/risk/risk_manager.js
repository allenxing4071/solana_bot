...
class RiskManager {
    ...
    loadBlacklist() {
        ...
        logger_1.default.info(`已加载 ${this.blacklistedTokens.size} 个黑名单代币`, MODULE_NAME);
    }
    /**
     * 检查是否存在紧急停止状态
     * 查询系统当前是否处于紧急状态
    ...
    triggerEmergencyStop(reason) {
        ...
        // 记录警告日志
        // 就像在航行日志中记录紧急情况
        logger_1.default.warn(`触发紧急停止: ${reason}`, MODULE_NAME);
        // 这里应该添加紧急通知逻辑
        // 例如发送邮件或短信通知
    }
    /**
    ...
    canTrade(positions) {
        ...
        if (stats.tradeCount >= this.tradingLimits.maxDailyTrades) {
            logger_1.default.warn(`交易被禁止: 已达每日最大交易次数 ${this.tradingLimits.maxDailyTrades}`, MODULE_NAME);
            return false;
        }
        // 检查是否超过每日投资金额限制
        // 就像检查是否超过每日资源投入限制
        if (stats.totalInvested >= this.tradingLimits.maxDailyInvestment) {
            logger_1.default.warn(`交易被禁止: 已达每日最大投资金额 $${this.tradingLimits.maxDailyInvestment}`, MODULE_NAME);
            return false;
        }
        // 检查是否超过最大持仓数量限制
        // 就像检查是否超过最大同时作业点限制
        if (positions.length >= this.tradingLimits.maxOpenPositions) {
            logger_1.default.warn(`交易被禁止: 已达最大持仓数量 ${this.tradingLimits.maxOpenPositions}`, MODULE_NAME);
            return false;
        }
        // 检查总风险敞口
        // 就像检查总体风险暴露程度
        ...
        if (totalExposure >= this.tradingLimits.maxTotalExposure) {
            logger_1.default.warn(`交易被禁止: 总风险敞口过高 $${totalExposure.toFixed(2)}`, MODULE_NAME);
            return false;
        }
        // 全部检查通过
        // 就像安全检查全部通过
     ...
     */
    calculateTokenRisk(token, opportunity) {
        // 默认风险级别
        let riskScore = 3;
        // 检查是否黑名单代币
        ...
        const tokenCreatedAt = tokenMetadata.createdAt || 0;
     ...
     */
    generateRiskReport(token, opportunity, positions) {
        // 检查缓存中是否已有最近的风险报告
        // 就像查找是否已有这片海域的评估报告
        ...
        // 计算整体风险
        // 就像综合所有因素确定总体安全等级
        const overallRisk = this.calculateOverallRisk(tokenRisk, marketRisk, liquidityRisk, exposureRisk);
        // 生成风险建议
        // 就像给出专业的操作建议
     ...
     */
    calculateExposureRisk(token, positions) {
        // 计算总敞口
        // 就像计算总投入资源
        const totalExposure = this.calculateTotalExposure(positions);
        // 计算对特定代币的敞口
     ...
     */
    calculateOverallRisk(tokenRisk, marketRisk, liquidityRisk, exposureRisk) {
        // 获取最高风险等级
        // 就像找出最危险的因素
     ...
     */
    allocateFunds(token, opportunity, positions) {
        // 生成风险报告
        // 就像获取安全专家的海域评估
    ...
    getRemainingDailyBudget() {
        ...
        if (!stats) {
            ...
            // 就像航行记录意外丢失时的应急措施
            logger_1.default.error('无法获取今日交易统计', MODULE_NAME);
            return 0;
        }
        // 计算剩余预算
        // 就像计算剩余可用燃料
     ...
     */
    recordTradeResult(result, amount) {
        const today = this.getDateString();
     ...
     * @param stats 每日统计数据
     */
    checkEmergencyConditions(stats) {
        // 检查失败率
     ...
     * @param reason 添加原因
     */
    addToBlacklist(mintAddress, reason) {
        const mintString = typeof mintAddress === 'string' ? mintAddress : mintAddress.toBase58();
        this.blacklistedTokens.add(mintString);
        logger_1.default.info(`已将代币添加到黑名单: ${mintString}`, MODULE_NAME, { reason });
        // 这里可以添加持久化存储逻辑
    }
    /**
     * 从黑名单移除代币
    ...
    removeFromBlacklist(mintAddress) {
        ...
        if (removed) {
            logger_1.default.info(`已从黑名单移除代币: ${mintString}`, MODULE_NAME);
            // 这里可以添加持久化存储逻辑
        }
    }
}
