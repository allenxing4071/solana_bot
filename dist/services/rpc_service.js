...
class RPCService {
    ...
    constructor() {
        ...
        try {
            ...
            const wsUrl = config_1.default.network.wsUrl;
            // 获取备用RPC端点
    ...
    switchToNextEndpoint() {
        ...
        try {
            ...
            logger_1.default.info(`已切换到备用RPC端点: ${newEndpoint}`, MODULE_NAME);
            return true;
        }
        catch (error) {
    ...
    async reconnect() {
        try {
            // 先尝试重新连接到主RPC
            const mainRpcUrl = config_1.default.network.rpcUrl;
            const wsUrl = config_1.default.network.wsUrl;
    ...
    async withRetry(fn, ...args) {
        ...
        for (let attempt = 0; attempt < MAX_CONN_RETRIES; attempt++) {
            try {
                ...
                lastError = error;
                logger_1.default.warn(`RPC调用失败，尝试重试 (${attempt + 1}/${MAX_CONN_RETRIES})`, MODULE_NAME, {
                    error: error instanceof Error ? error.message : String(error)
                });
                // 检查是否是连接错误，如果是则尝试重连
    ...
    async simulateTransaction(transaction, signers) {
        return this.withRetry(async () => {
            ...
            if (transaction instanceof web3_js_1.Transaction) {
                ...
                result = await this._connection.simulateTransaction(transaction);
            }
            if (result.value.err) {
    ...
    async getAccountInfo(address, commitment) {
        return this.withRetry(async () => {
            return await this._connection.getAccountInfo(address, commitment || connectionConfig.commitment);
        });
    }
    /**
     * 获取多个账户信息
    ...
    async getMultipleAccountsInfo(addresses, commitment) {
        return this.withRetry(async () => {
            return await this._connection.getMultipleAccountsInfo(addresses, commitment || connectionConfig.commitment);
        });
    }
    /**
     * 获取程序账户
    ...
    async getProgramAccounts(programId, commitment) {
        try {
            ...
            return await this.withRetry(async () => {
                ...
                logger_1.default.debug(`获取到 ${accounts.length} 个程序账户`, MODULE_NAME, {
            ...
            logger_1.default.error(`获取程序账户失败: ${programId.toBase58()}`, MODULE_NAME, {
                error: error instanceof Error ? error.message : String(error)
            });
            // 出错时返回空数组，避免整个流程崩溃
            return [];
    ...
    async subscribeAccount(address, callback, commitment) {
        ...
        const subscriptionId = await wsConn.onAccountChange(address, callback, commitment || connectionConfig.commitment);
        // 保存订阅ID以便后续管理
        this._wsSubscriptions.set(address.toBase58(), subscriptionId);
        logger_1.default.debug(`已订阅账户 ${address.toBase58()}`, MODULE_NAME);
    ...
    async subscribeProgram(programId, callback, commitment) {
        ...
        const subscriptionId = await wsConn.onProgramAccountChange(programId, callback, commitment || connectionConfig.commitment);
        // 保存订阅ID以便后续管理
        this._wsSubscriptions.set(`program:${programId.toBase58()}`, subscriptionId);
        logger_1.default.debug(`已订阅程序 ${programId.toBase58()}`, MODULE_NAME);
        return subscriptionId;
    }
    /**
     * 订阅日志
    ...
    async subscribeLogs(filter, callback) {
        ...
        const subscriptionId = await wsConn.onLogs(filter, callback);
        // 保存订阅ID以便后续管理
        let filterKey;
        ...
        logger_1.default.debug(`已订阅日志 ${filterKey}`, MODULE_NAME);
        return subscriptionId;
    }
    /**
     * 取消订阅
    ...
    async unsubscribe(subscriptionId) {
        ...
        try {
            ...
            logger_1.default.warn('取消订阅失败', MODULE_NAME, { error: error instanceof Error ? error.message : String(error) });
            return false;
        }
    }
    /**
    ...
    async unsubscribeAll() {
        ...
        for (const id of subscriptionIds) {
            try {
                ...
                logger_1.default.warn(`无法取消订阅ID ${id}`, MODULE_NAME, { error: error instanceof Error ? error.message : String(error) });
            }
        }
