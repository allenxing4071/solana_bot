/**
 * 机会检测器
 * 负责分析新池子，评估交易机会，计算价格和滑点
 */

import { PublicKey } from '@solana/web3.js';
import logger from '../../core/logger';
import { PoolInfo, TokenInfo, TradingOpportunity, DexType } from '../../core/types';
import tokenValidator from './token_validator';
import appConfig from '../../core/config';

const MODULE_NAME = 'OpportunityDetector';

/**
 * 池子分析结果接口
 */
interface PoolAnalysisResult {
  isValid: boolean;
  tokenInfo?: TokenInfo;
  estimatedPriceUsd?: number;
  liquidityUsd?: number;
  confidenceScore: number;
  reason?: string;
}

/**
 * 机会检测器类
 * 负责检测并分析交易机会
 */
export class OpportunityDetector {
  // 基础交易代币(SOL或USDC)
  private baseTokens: TokenInfo[] = [];
  // 最小要求流动性(美元)
  private minLiquidityUsd: number;
  // 最大初始价格(美元)
  private maxInitialPriceUsd: number;
  
  /**
   * 构造函数
   */
  constructor() {
    // 从配置中加载设置
    this.minLiquidityUsd = appConfig.trading.tokenValidation.minLiquidityUsd;
    this.maxInitialPriceUsd = appConfig.trading.tokenValidation.maxInitialPriceUsd;
    
    // 初始化基础代币列表 (通常是SOL和USDC)
    this.initializeBaseTokens();
    
    logger.info('机会检测器初始化完成', MODULE_NAME);
  }
  
  /**
   * 初始化基础代币列表
   */
  private initializeBaseTokens(): void {
    // SOL代币信息
    this.baseTokens.push({
      mint: new PublicKey('So11111111111111111111111111111111111111112'), // 包装后的SOL地址
      symbol: 'SOL',
      name: 'Solana',
      decimals: 9,
      isVerified: true,
      isTrusted: true
    });
    
    // USDC代币信息
    this.baseTokens.push({
      mint: new PublicKey('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v'), // USDC地址
      symbol: 'USDC',
      name: 'USD Coin',
      decimals: 6,
      isVerified: true,
      isTrusted: true
    });
  }
  
  /**
   * 分析新池子并检测交易机会
   * @param poolInfo 池子信息
   * @returns 交易机会或null(如果没有有效机会)
   */
  async detectOpportunity(poolInfo: PoolInfo): Promise<TradingOpportunity | null> {
    logger.debug(`分析池子: ${poolInfo.address.toBase58()}`, MODULE_NAME, {
      dex: poolInfo.dex,
      tokenA: poolInfo.tokenAMint.toBase58(),
      tokenB: poolInfo.tokenBMint.toBase58()
    });
    
    try {
      // 1. 确定目标代币和基础代币
      const [targetToken, baseToken] = await this.identifyTokens(poolInfo);
      
      if (!targetToken || !baseToken) {
        logger.debug(`跳过池子: 无法识别目标代币或基础代币`, MODULE_NAME, {
          poolAddress: poolInfo.address.toBase58()
        });
        return null;
      }
      
      // 2. 验证目标代币
      const validationResult = await tokenValidator.validateToken(targetToken);
      
      if (!validationResult.isValid) {
        logger.debug(`跳过池子: 代币验证失败 - ${validationResult.reason}`, MODULE_NAME, {
          token: targetToken.symbol || targetToken.mint.toBase58()
        });
        return null;
      }
      
      // 3. 分析池子
      const analysis = await this.analyzePool(poolInfo, targetToken, baseToken);
      
      if (!analysis.isValid) {
        logger.debug(`跳过池子: 分析失败 - ${analysis.reason}`, MODULE_NAME, {
          token: targetToken.symbol || targetToken.mint.toBase58()
        });
        return null;
      }
      
      // 4. 如果所有检查都通过，创建交易机会
      const priorityScore = this.calculatePriorityScore(analysis, poolInfo, targetToken);
      
      // 获取预估的输出数量
      const estimatedOutAmount = this.estimateOutputAmount(
        poolInfo,
        baseToken,
        targetToken,
        appConfig.trading.buyStrategy.maxAmountPerTrade
      );
      
      // 创建交易机会对象
      const opportunity: TradingOpportunity = {
        pool: poolInfo,
        targetToken,
        baseToken,
        estimatedPriceUsd: analysis.estimatedPriceUsd,
        estimatedSlippage: this.estimateSlippage(poolInfo, analysis.liquidityUsd),
        confidence: analysis.confidenceScore,
        liquidityUsd: analysis.liquidityUsd,
        action: 'buy',
        priorityScore,
        timestamp: Date.now(),
        estimatedOutAmount
      };
      
      logger.info(`检测到交易机会: ${targetToken.symbol || targetToken.mint.toBase58()}`, MODULE_NAME, {
        dex: poolInfo.dex,
        price: analysis.estimatedPriceUsd,
        score: priorityScore.toFixed(2),
        liquidity: analysis.liquidityUsd
      });
      
      return opportunity;
    } catch (error) {
      logger.error(`分析池子时出错: ${poolInfo.address.toBase58()}`, MODULE_NAME, {
        error: error instanceof Error ? error.message : String(error)
      });
      return null;
    }
  }
  
  /**
   * 识别目标代币和基础代币
   * @param poolInfo 池子信息
   * @returns [目标代币, 基础代币] 或 [null, null]
   */
  private async identifyTokens(poolInfo: PoolInfo): Promise<[TokenInfo | null, TokenInfo | null]> {
    const tokenAIsBase = this.isBaseToken(poolInfo.tokenAMint);
    const tokenBIsBase = this.isBaseToken(poolInfo.tokenBMint);
    
    // 如果两个都是基础代币或两个都不是基础代币，则跳过
    if (tokenAIsBase === tokenBIsBase) {
      return [null, null];
    }
    
    let targetTokenMint: PublicKey;
    let baseTokenMint: PublicKey;
    
    if (tokenAIsBase) {
      baseTokenMint = poolInfo.tokenAMint;
      targetTokenMint = poolInfo.tokenBMint;
    } else {
      baseTokenMint = poolInfo.tokenBMint;
      targetTokenMint = poolInfo.tokenAMint;
    }
    
    // 获取基础代币信息
    const baseToken = this.getBaseTokenInfo(baseTokenMint);
    
    if (!baseToken) {
      return [null, null];
    }
    
    // 获取或加载目标代币信息
    const targetToken: TokenInfo = {
      mint: targetTokenMint,
      symbol: poolInfo.tokenASymbol || poolInfo.tokenBSymbol || undefined
    };
    
    // 丰富代币信息
    const enrichedTargetToken = await tokenValidator.enrichTokenInfo(targetToken);
    
    return [enrichedTargetToken, baseToken];
  }
  
  /**
   * 判断是否为基础代币
   * @param mint 代币Mint地址
   * @returns 是否为基础代币
   */
  private isBaseToken(mint: PublicKey): boolean {
    const mintString = mint.toBase58();
    return this.baseTokens.some(token => token.mint.toBase58() === mintString);
  }
  
  /**
   * 获取基础代币信息
   * @param mint 代币Mint地址
   * @returns 基础代币信息或null
   */
  private getBaseTokenInfo(mint: PublicKey): TokenInfo | null {
    const mintString = mint.toBase58();
    return this.baseTokens.find(token => token.mint.toBase58() === mintString) || null;
  }
  
  /**
   * 分析池子
   * @param poolInfo 池子信息
   * @param targetToken 目标代币
   * @param baseToken 基础代币
   * @returns 池子分析结果
   */
  private async analyzePool(
    poolInfo: PoolInfo, 
    targetToken: TokenInfo, 
    baseToken: TokenInfo
  ): Promise<PoolAnalysisResult> {
    // 这里实际实现会更复杂，需要查询池子数据、分析流动性等
    // 以下为简化实现
    
    // 预估价格(美元)
    const estimatedPriceUsd = await this.estimateTokenPrice(poolInfo, targetToken, baseToken);
    
    // 预估流动性(美元)
    const liquidityUsd = await this.estimateLiquidity(poolInfo, targetToken, baseToken);
    
    // 检查价格是否在合理范围内
    if (estimatedPriceUsd > this.maxInitialPriceUsd) {
      return {
        isValid: false,
        confidenceScore: 0,
        reason: `价格过高: $${estimatedPriceUsd} > $${this.maxInitialPriceUsd}`
      };
    }
    
    // 检查流动性是否足够
    if (liquidityUsd < this.minLiquidityUsd) {
      return {
        isValid: false,
        confidenceScore: 0,
        reason: `流动性不足: $${liquidityUsd} < $${this.minLiquidityUsd}`
      };
    }
    
    // 计算信心分数
    const confidenceScore = this.calculateConfidenceScore(poolInfo, targetToken, estimatedPriceUsd, liquidityUsd);
    
    return {
      isValid: true,
      tokenInfo: targetToken,
      estimatedPriceUsd,
      liquidityUsd,
      confidenceScore
    };
  }
  
  /**
   * 估算代币价格
   * @param poolInfo 池子信息
   * @param targetToken 目标代币
   * @param baseToken 基础代币
   * @returns 估算的美元价格
   */
  private async estimateTokenPrice(
    poolInfo: PoolInfo, 
    targetToken: TokenInfo, 
    baseToken: TokenInfo
  ): Promise<number> {
    // 实际实现需要查询池子数据和基础代币价格
    // 这里使用模拟值
    
    // 基础模拟价格
    let basePrice = 0;
    
    // 根据基础代币设置基准价格
    if (baseToken.symbol === 'SOL') {
      basePrice = 100; // 假设SOL价格为$100
    } else if (baseToken.symbol === 'USDC') {
      basePrice = 1; // USDC = $1
    }
    
    // 根据不同DEX使用不同的价格估算逻辑
    switch (poolInfo.dex) {
      case DexType.RAYDIUM:
        // Raydium池子价格估算逻辑
        return basePrice * (0.0001 + Math.random() * 0.01);
        
      case DexType.ORCA:
        // Orca池子价格估算逻辑
        return basePrice * (0.0002 + Math.random() * 0.02);
        
      default:
        // 默认估算逻辑
        return basePrice * (0.0001 + Math.random() * 0.01);
    }
  }
  
  /**
   * 估算池子流动性
   * @param poolInfo 池子信息
   * @param targetToken 目标代币
   * @param baseToken 基础代币
   * @returns 估算的美元流动性
   */
  private async estimateLiquidity(
    poolInfo: PoolInfo, 
    targetToken: TokenInfo, 
    baseToken: TokenInfo
  ): Promise<number> {
    // 实际实现需要查询池子数据
    // 这里使用模拟值
    
    // 根据不同DEX使用不同的流动性估算逻辑
    switch (poolInfo.dex) {
      case DexType.RAYDIUM:
        // Raydium池子流动性估算逻辑
        return 1000 + Math.random() * 10000;
        
      case DexType.ORCA:
        // Orca池子流动性估算逻辑
        return 1500 + Math.random() * 15000;
        
      default:
        // 默认估算逻辑
        return 1000 + Math.random() * 5000;
    }
  }
  
  /**
   * 估算交易滑点
   * @param poolInfo 池子信息
   * @param liquidityUsd 流动性(美元)
   * @returns 估算的滑点百分比
   */
  private estimateSlippage(poolInfo: PoolInfo, liquidityUsd?: number): number {
    // 如果没有流动性数据，使用默认值
    if (!liquidityUsd) {
      return 5; // 默认5%滑点
    }
    
    // 根据流动性估算滑点
    if (liquidityUsd > 50000) {
      return 1; // >$50k流动性，预计1%滑点
    } else if (liquidityUsd > 10000) {
      return 2; // >$10k流动性，预计2%滑点
    } else if (liquidityUsd > 5000) {
      return 3; // >$5k流动性，预计3%滑点
    } else {
      return 5; // 低流动性，预计5%滑点
    }
  }
  
  /**
   * 计算信心分数
   * @param poolInfo 池子信息
   * @param tokenInfo 代币信息
   * @param price 预估价格
   * @param liquidity 预估流动性
   * @returns 信心分数(0-1)
   */
  private calculateConfidenceScore(
    poolInfo: PoolInfo,
    tokenInfo: TokenInfo,
    price: number,
    liquidity: number
  ): number {
    // 基础分数
    let score = 0.5;
    
    // 根据代币信息调整
    if (tokenInfo.isVerified) {
      score += 0.2;
    }
    
    if (tokenInfo.metadata && tokenInfo.metadata.name && tokenInfo.metadata.symbol) {
      score += 0.1;
    }
    
    // 根据价格调整
    if (price < 0.0001) { // 超低价格通常是垃圾币
      score -= 0.2;
    } else if (price < 0.001) {
      score -= 0.1;
    }
    
    // 根据流动性调整
    if (liquidity > 50000) {
      score += 0.2;
    } else if (liquidity > 10000) {
      score += 0.1;
    } else if (liquidity < 2000) {
      score -= 0.1;
    }
    
    // 确保分数在0-1范围内
    return Math.max(0, Math.min(1, score));
  }
  
  /**
   * 计算优先级分数
   * @param analysis 池子分析结果
   * @param poolInfo 池子信息
   * @param tokenInfo 代币信息
   * @returns 优先级分数(0-1)
   */
  private calculatePriorityScore(
    analysis: PoolAnalysisResult,
    poolInfo: PoolInfo,
    tokenInfo: TokenInfo
  ): number {
    // 基础分数为信心分数
    let score = analysis.confidenceScore || 0.5;
    
    // 根据首次检测时间调整(越新越好)
    const ageInSeconds = (Date.now() - poolInfo.firstDetectedAt) / 1000;
    
    if (ageInSeconds < 10) { // 10秒内
      score += 0.3;
    } else if (ageInSeconds < 30) { // 30秒内
      score += 0.2;
    } else if (ageInSeconds < 60) { // 1分钟内
      score += 0.1;
    } else if (ageInSeconds > 300) { // 5分钟以上
      score -= 0.2;
    }
    
    // 根据DEX类型调整优先级
    if (poolInfo.dex === DexType.RAYDIUM) {
      score += 0.05; // Raydium略微加分
    }
    
    // 如果代币已在白名单中
    if (tokenInfo.isTrusted) {
      score += 0.3;
    }
    
    // 确保分数在0-1范围内
    return Math.max(0, Math.min(1, score));
  }
  
  /**
   * 估算输出代币数量
   * @param poolInfo 池子信息
   * @param inputToken 输入代币
   * @param outputToken 输出代币
   * @param amountInSol 输入SOL金额
   * @returns 估计的输出代币数量
   */
  private estimateOutputAmount(
    poolInfo: PoolInfo,
    inputToken: TokenInfo,
    outputToken: TokenInfo,
    amountInSol: number
  ): bigint {
    // 实际实现需要根据池子数据计算
    // 这里使用简化的模拟值
    
    // 模拟的输入金额(以SOL为单位)
    const inputAmount = BigInt(Math.floor(amountInSol * 1e9)); // 1 SOL = 10^9 lamports
    
    // 模拟的输出比例(每SOL获得多少代币)
    let outputRatio = 1000000; // 假设每SOL获得100万代币
    
    // 根据DEX类型调整比例
    if (poolInfo.dex === DexType.RAYDIUM) {
      outputRatio = 1200000; // Raydium给更多代币
    } else if (poolInfo.dex === DexType.ORCA) {
      outputRatio = 900000; // Orca给稍少代币
    }
    
    // 模拟的输出金额
    const estimatedOutput = (inputAmount * BigInt(outputRatio)) / BigInt(1e9);
    
    return estimatedOutput;
  }
}

// 导出单例实例
const opportunityDetector = new OpportunityDetector();
export default opportunityDetector; 