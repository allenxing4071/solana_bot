...
interface TradeExecutionParams {
  opportunity: TradingOpportunity;  // 交易机会，就像发现的鱼群信息
  wallet?: Keypair;                 // 钱包密钥对，就像指定使用哪艘小艇去捕捞
  maxRetries?: number;              // 最大重试次数，就像最多允许投网的次数
  confirmTimeout?: number;          // 确认超时时间，就像等待渔网回收的最长时间
}
...
export class TraderExecutor {
  private connection: Connection; // 区块链连接，就像与海港的通信设备
  private txBuilder: any; // 交易构建器，就像捕鱼装备的制作工具
  private walletKeypair: Keypair; // 钱包密钥对，就像船长的身份证和钥匙

  // 交易配置参数
  // 就像捕捞行动的基本规则
  private readonly maxRetries: number; // 最大重试次数，就像最多允许的投网次数
  private readonly confirmTimeout: number; // 确认超时，就像等待渔网回收的最长时间
  private readonly priorityFeeEnabled: boolean; // 优先费用，就像是否使用高级捕鱼装备
  private readonly maxSlippageBps: number; // 最大滑点，就像可接受的捕获量误差范围
  ...
  async executeBuy(params: TradeExecutionParams): Promise<TradeResult> {
    ...
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : String(error),
        errorCode: error instanceof SendTransactionError ? 
          (error as unknown as { code: string }).code : undefined,
        timestamp: Date.now()
      };
    }
  }
  ...
  async getTransactionStatus(signature: string): Promise<ConfirmedTransaction | null> {
    try {
      ...
      // 转换交易信息类型
      const txResponse = txInfo as TransactionResponse;
      const transaction = Transaction.from(txResponse.transaction.message.serialize());
