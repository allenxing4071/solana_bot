...
class WalletManager {
  ...
  constructor(privateKey: string | Uint8Array) {
    try {
      ...
      // 处理错误
      // 就像处理初始化过程中的意外情况
      logger.error('钱包管理器初始化失败', MODULE_NAME, error);
      throw new Error('钱包管理器初始化失败: ' + (error as Error).message);
    }
  }
   ...
   */
  private loadWalletFromConfig(privateKey: string | Uint8Array) {
    try {
      // 检查配置是否包含私钥
      // 就像检查是否有既定的保险箱钥匙
      if (privateKey) {
        ...
        // 创建钱包密钥对
        // 就像制作可用的保险箱钥匙和身份证
        this.tradingWallet = Keypair.fromSecretKey(privateKeyArray);
        
        logger.info('已从配置加载交易钱包', MODULE_NAME);
      } else {
        // 如果配置中没有私钥，生成新钱包
        // 就像制作全新的保险箱和钥匙
        logger.warn('配置中未找到钱包私钥，将生成新钱包', MODULE_NAME);
        this.tradingWallet = Keypair.generate();
        
        // 将新生成的私钥保存到配置中
        // 就像将新钥匙信息记录在安全文件中
        const privateKeyBase64 = Buffer.from(
          this.tradingWallet.secretKey
        ).toString('base64');
        
        // 在实际应用中，这里应该更新配置文件
        logger.info('已生成新交易钱包', MODULE_NAME);
      }
      
      // 记录钱包公钥
      // 就像记录船长的身份证号码
      logger.info(
        `交易钱包公钥: ${this.tradingWallet.publicKey.toString()}`,
        MODULE_NAME
      );
    } catch (error) {
      // 处理错误
      // 就像处理钥匙出问题的情况
      logger.error('加载钱包失败', MODULE_NAME, error);
      throw error;
    }
  }
  ...
  async getSOLBalance(address?: PublicKey): Promise<number> {
    try {
      ...
      logger.debug(
        `地址 ${targetAddress.toString()} 的SOL余额: ${solBalance}`,
        MODULE_NAME
      );
      ...
      // 处理错误
      // 就像处理通信故障
      logger.error('获取SOL余额失败', MODULE_NAME, error);
      throw error;
    }
  }
  ...
  async getTokenBalance(
    ...
    try {
      ...
      // 获取关联代币账户地址
      // 就像确定仓库位置
      const tokenAccountAddress = await import('@solana/spl-token').then(splToken => 
      ...
      try {
        ...
        logger.debug(
          `地址 ${ownerAddress.toString()} 的代币 ${token.symbol} 余额: ${balance}`,
          MODULE_NAME
        );
        ...
        // 就像这种鱼可能还没有专门的仓库
        logger.debug(
          `代币账户 ${tokenAccountAddress.toString()} 不存在或余额为0`,
          MODULE_NAME
        );
        return 0;
      }
      ...
      // 处理错误
      // 就像处理检查过程中的意外情况
      logger.error('获取代币余额失败', MODULE_NAME, error);
      throw error;
    }
  }
  ...
  async transferSol(
    ...
    try {
      ...
      logger.info(
        `转移 ${amountSol} SOL 到 ${toAddress.toString()} 成功，交易ID: ${signature}`,
        MODULE_NAME
      );
      ...
      // 处理错误
      // 就像处理转移过程中的意外情况
      logger.error('转移SOL失败', MODULE_NAME, error);
      throw error;
    }
  }
  ...
  async createTokenAccount(
    ...
    try {
      ...
      // 获取关联代币账户地址
      // 就像确定新仓库的位置
      const associatedTokenAddress = await import('@solana/spl-token').then(splToken => 
        splToken.getAssociatedTokenAddressSync(
          mintAddress,
          ownerAddress,
          false,
      ...
      try {
        ...
        logger.debug(
          `代币账户已存在: ${associatedTokenAddress.toString()}`,
          MODULE_NAME
        );
      ...
      // 创建关联代币账户的交易指令
      // 就像准备仓库建造图纸
      const createATAInstruction = await import('@solana/spl-token').then(splToken => 
        splToken.createAssociatedTokenAccountInstruction(
          this.tradingWallet.publicKey, // 支付费用的账户
      ...
      logger.info(
        `为代币 ${mintAddress.toString()} 创建账户成功: ${associatedTokenAddress.toString()}`,
        MODULE_NAME
      );
      ...
      // 处理错误
      // 就像处理建造过程中的意外情况
      logger.error('创建代币账户失败', MODULE_NAME, error);
      throw error;
    }
  }
}
