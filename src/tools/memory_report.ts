 ...
 * @returns 内存使用统计数据
 */
function collectMemoryStats(): MemoryStats {
  // 系统内存信息
 ...
 * @returns 格式化的内存报告字符串
 */
function generateDetailedReport(stats: MemoryStats): string {
 ...
 * @returns 状态描述
 */
function getMemoryHealthStatus(stats: MemoryStats): string {
 ...
 * @returns 保存的文件路径
 */
async function saveReportToFile(report: string): Promise<string> {
  // 确保目录存在
